...
- Random Walk in $$R^{\text{latent dim}}$$: We create a family of Heston volatility surfaces by performing a random walk in the latent space of the Heston model. 
- Fitting Randomly Generated Surfaces: We test the VAE's capability to generate a volatility surface that closely fits a randomly generated target surface. This involves optimizing within the latent space to minimize the difference between the VAE-generated surface and the target surface.
